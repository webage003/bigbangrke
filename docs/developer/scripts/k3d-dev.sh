...
mkdir -p ~/aws
# XXX "InstanceInitiatedShutdownBehavior":"Terminate",
...
# INFO: Old spot request
# SIR=`aws ec2 request-spot-instances \
#  --output json --no-cli-pager \
#  --instance-count 1 \
# XXX:  --attribute InstanceInitiatedShutdownBehavior=Terminate \
# XXX:  --instance-initiated-shutdown-behavior terminate \
...
then
  ...
  then
    ...
    ssh -i ~/.ssh/"${KeyName}".pem -o StrictHostKeyChecking=no ubuntu@"${PublicIP}" \
      "k3d cluster create \
      --servers 1 \
      --agents 3 \
      --volume /etc/machine-id:/etc/machine-id@server:0 \
      --volume /etc/machine-id:/etc/machine-id@agent:0,1,2 \
      --k3s-arg \"--disable=traefik@server:0\" \
      --k3s-arg \"--disable=metrics-server@server:0\" \
      --k3s-arg \"--tls-san=${PrivateIP}@server:0\" \
    ...
    ssh -i ~/.ssh/"${KeyName}".pem -o StrictHostKeyChecking=no ubuntu@"${PublicIP}" \
      ...
      --k3s-arg \"--disable=traefik@server:0\" \
      --k3s-arg \"--disable=metrics-server@server:0\" \
      --k3s-arg \"--tls-san=${PublicIP}@server:0\" \
      --k3s-arg \"--disable=servicelb@server:0\" \
      --network k3d-network \
      --port 80:80@loadbalancer \
      --port 443:443@loadbalancer \
